...
# pyre-unsafe
"""BGP_AND_FBOSS_PLATFORM_HARDENING self-entry — Non-CICD residue TestConfig.

Note: the original `test_config_name` collides with the surrounding source list
constant `BGP_AND_FBOSS_PLATFORM_HARDENING_TEST_CONFIGS`, so this module exposes
a uniquely-named Python constant `BGP_AND_FBOSS_PLATFORM_HARDENING_SELF_ENTRY_TEST_CONFIG`
while preserving the `test_config_name` string verbatim.
...
BGP_AND_FBOSS_PLATFORM_HARDENING_SELF_ENTRY_TEST_CONFIG = (
    test_config_for_bgp_and_fboss_platform_hardening_in_conveyor(
        test_config_name="BGP_AND_FBOSS_PLATFORM_HARDENING_TEST_CONFIGS",
        device_name="fsw002.p003.f01.qzd1",
