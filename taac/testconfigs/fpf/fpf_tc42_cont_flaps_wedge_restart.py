...
"""TC42: Continuous interface flaps + parallel wedge_agent restart every 2 min.
...
``wedge_agent`` on every flapped GTSW every 2 minutes, in parallel, for the whole
flap window. Maximum control-plane + link chaos. After everything stops, the
stable-state longevity playbook (same expectations as fpf_stress_test_config)
validates full recovery.

Usage:
  TAAC_SSH_VIA_LAB_SSH=1 buck2 run neteng/netcastle:netcastle_taac -- \\
    --team taac --test-config fpf_tc42_cont_flaps_wedge_restart \\
...
def create_fpf_tc42_test_config() -> TestConfig:
    ...
    disrupt_playbook = create_fpf_disrupt_window_playbook(
        ...
        playbook_name="fpf_tc42_cont_flaps_wedge_restart_disrupt",
    )
    ...
    longevity_playbook = create_fpf_hardening_playbook_v2(
        ...
        prefix_count=PREFIX_COUNT,
        community_list=DEFAULT_COMMUNITY_LIST,
        playbook_name="fpf_tc42_cont_flaps_wedge_restart_longevity",
    ...
    return TestConfig(
        name="fpf_tc42_cont_flaps_wedge_restart",
...
TEST_CONFIG = create_fpf_tc42_test_config()
