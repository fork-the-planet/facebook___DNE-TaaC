...
"""TC43: Continuous interface flaps + parallel bgpd restart every 2 min.
...
``bgpd`` on every flapped GTSW every 2 minutes, in parallel, for the whole flap
...
Usage:
  TAAC_SSH_VIA_LAB_SSH=1 buck2 run neteng/netcastle:netcastle_taac -- \\
    --team taac --test-config fpf_tc43_cont_flaps_bgp_restart \\
...
CHURN_SERVICE = taac_types.Service.BGP
...
def create_fpf_tc43_test_config() -> TestConfig:
    ...
    disrupt_playbook = create_fpf_disrupt_window_playbook(
        ...
        playbook_name="fpf_tc43_cont_flaps_bgp_restart_disrupt",
    )
    ...
    longevity_playbook = create_fpf_hardening_playbook_v2(
        ...
        prefix_count=PREFIX_COUNT,
        community_list=DEFAULT_COMMUNITY_LIST,
        playbook_name="fpf_tc43_cont_flaps_bgp_restart_longevity",
    ...
    return TestConfig(
        name="fpf_tc43_cont_flaps_bgp_restart",
...
TEST_CONFIG = create_fpf_tc43_test_config()
