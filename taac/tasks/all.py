...
# `async_get_memory_current_pct` has an OSS-safe stub in system_stress_utils that
# raises NotImplementedError, so it is imported unconditionally. It is called at
# runtime by `AllocateCgroupSliceMemory.run()`, which is registered in the OSS
# TASK_REGISTRY; gating the import would surface a confusing NameError under
# TAAC_OSS=1 instead of the intended NotImplementedError.
...
class CreateVipInjectors(BaseTask):
    ...
    async def inject_vips(
        ...
        if TAAC_OSS:
            raise NotImplementedError(
                "inject_vips requires the Meta-internal "
    ...
    async def get_all_injector_ids(
        ...
        if TAAC_OSS:
            raise NotImplementedError(
                "get_all_injector_ids requires the Meta-internal "
                "libfb.py.asyncio.thrift.get_direct_client and cannot run "
                "under TAAC_OSS=1."
            )
        async with get_direct_client(
