...
from taac.libs.traffic_generator import (
    DEFAULT_DEVICE_GROUP_CONFIG,
    TrafficGenerator,
)
...
class OtgTrafficGenerator(TrafficGenerator):
    ...
    async def async_create_ixia_setup(self) -> None:
        try:
            ...
            self.logger.info("[OTG] Starting OTG setup (push config → ARP → flows)...")
    ...
    async def _async_create_otg_port_configs(
        ...
        for conn in endpoint.direct_ixia_connections or []:
            ...
            for device_group_config in basic_port_config.device_group_configs or []:
                ...
                if device_group_config.v4_bgp_config:
                    thrift_v4_bgp_config = await self.async_create_bgp_config_thrift(
                        endpoint.name,
                        device_group_config.v4_bgp_config,
                        none_throws(thrift_ip_addresses_config.ipv4_addresses_config),
                        ixia_types.IpAddressFamily.IPV4,
                    )
                if device_group_config.v6_bgp_config:
                    thrift_v6_bgp_config = await self.async_create_bgp_config_thrift(
                        endpoint.name,
                        device_group_config.v6_bgp_config,
                        none_throws(thrift_ip_addresses_config.ipv6_addresses_config),
