...
class TestPortChannelExpectedStateHealthCheckFBOSS(
    ...
    async def test_port_channel_up_expected_up_returns_pass(self):
        ...
        result = await self.health_check._run(
            self.device, self.input, {"port_channel_name": "Port-Channel1"}
        )
        self.assertEqual(result.status, hc_types.HealthCheckStatus.PASS)
        self.assertIn("UP as expected", result.message)
    ...
    async def test_port_channel_not_found_returns_fail(self):
        ...
        result = await self.health_check._run(
            self.device, self.input, {"port_channel_name": "Port-Channel99"}
        )
        self.assertEqual(result.status, hc_types.HealthCheckStatus.FAIL)
    ...
    async def test_port_channel_down_expected_down_returns_pass(self):
        ...
        result = await self.health_check._run(
            self.device,
            self.input,
            {"port_channel_name": "Port-Channel1", "expected_up": False},
        )
        self.assertEqual(result.status, hc_types.HealthCheckStatus.PASS)
        self.assertIn("DOWN as expected", result.message)
